...
trigger.addEventListener('mousedown', () => {push_trigger(); roll();});
...
function roll_path_1(marble) {
    ...
    if (turner.flipped == 1 && !marble.turnerTriggered && marble.progress >= 0.74) {
        turn(turner);
...
function roll_path_5(marble) {
  ...
  if (!marble.rightTriggered && marble.progress >= 0.4) {
    turn(right_flip_flop);
    turn(middle_flip_flop);
    marble.middleTriggered = false;
    marble.rightTriggered = true;
...
function roll_path_8(marble) {
    ...
    if(marble.equalizerTriggered == false && marble.progress >= 0.55){
        turn(equalizer);
        marble.equalizerTriggered = true;
    }
